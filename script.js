document.addEventListener('DOMContentLoaded', () => {
  ...
  function generateTiles() {
    const doubledValues = [...tileValues, ...tileValues];
    const shuffledValues = doubledValues.sort(() => Math.random() - 0.5);
    gameContainer.innerHTML = '';
    tiles = shuffledValues.map((value, index) => {
  ...
  function flipTile(tile) {
    if (lockBoard || tile === firstTile || tile.classList.contains('flipped')) return;
...
// Touch event handlers for puzzle pieces
...
// Add touch event listeners to puzzle pieces
